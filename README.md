...
同类做法里有一种是把 Node（甚至整套 proot + Ubuntu）打包进 APK，好处很直接：**装上就能用**。
本项目刻意不做这件事，理由三条：

- **许可证与合规**：那类方案要连同第三方运行时的许可义务一起分发；本项目让用户自己去 F-Droid 装 **官方版 Termux**，义务留在上游与用户之间。
- **体积与更新速度**：本 APK 约 **12 MB**（不含任何运行时）。DSH 上游更新很勤，走"Termux 里 `npm` 升级"这条路，本 app 不必跟着发版。
- **安全面**：APK 里少装一整套运行时，就少一整套需要跟着修的东西；薄壳自身只暴露一个 WebView。

代价也说实话：**首次要自己跑一次 Termux 脚本（几分钟）**，`docs/04` 整篇就是为这一步写的；
如果你更希望"装上就用、不碰终端"，那它就不合适 —— 这只是取舍，不分好坏。

### 怎么挑一个手机端（给判断标准，不点评别人）

生态里已经有不少同类尝试，各有取向。省事的做法不是看 star 数，而是先问自己四个问题：

| 先问自己 | 影响你选哪一类 |
|---|---|
| 愿意在手机上装 Termux 吗？ | 不愿意 → 找"内嵌运行时"那类；愿意 → 薄壳更轻（本项目：12 MB、不打包运行时） |
| 需要"免 root 操作手机"（截图 / 点击 / 开应用）吗？ | 需要 → 得选带 Shizuku / Magisk 能力的那类；**本项目不做这件事** |
| 会不会在外面用（不在家里网段）？ | 会 → 需要"远程 / 组网"那一层；本项目给的是 SSH 隧道与回环代理两种做法 + `docs/05` 一整篇配方，但**不内建组网客户端** |
| 在意维护者会不会回你 issue 吗？ | 在意 → 看最近一次提交时间与 issue 回复；这个方向热度来得猛，选题一样、寿命差别很大 |

**本项目的四条取舍写在明处**：不内嵌运行时（体积与安全面优先）、不做免 root 操作、不内建组网客户端、UI 中文优先。
四条里有你必需的那条，就别选它 —— 反过来也一样：**你要的就是"轻、可读、不带运行时"，那它才合适。**

### 这个壳真解决过的问题（都留了证据）

它不是"又一个 WebView 壳"。下面每条都是真机上踩出来、并把原因与修法写进文档的：

- **系统把 Termux 杀掉 / 拦住"别的应用启动 Termux"**：唤醒锁 + 「打开 Termux 就自动拉起 DSH」的 `~/.bashrc` 钩子；
  app 侧把"成功"的判据换成**实测端口探活**（`core/TermuxStartReport`，有单测），失败时**不说假话**并给「打开 Termux」。见 `docs/04` §三。
- **手机那份 DSH 没有云 API 的 key**（而它的默认模型偏偏是云端的）：定位、搬运与验证命令写成一节。见 `docs/04` §六。
- **离开家里网段就用不了**：Tailscale 组网 + `tailscale serve` 真证书入口 + 用本机持久密钥签 30 天凭据。见 `docs/05` 全篇。
- **本地模型窗口比声明的小 → 长会话被打成 HTTP 500**：定位到"消息列表被裁得只剩工具返回 + 模型自带模板的硬守卫"，
  把窗口对齐后修复；排查过程留在 `CHANGELOG.md`。
...
├─ tools/                   构建与验证脚本（见下表）
├─ docs/                    说明书 + 图片 + 排障
├─ AGENTS.md                在这个仓库里改代码的规则（人也可以读）
└─ dist/ · phone/           构建产物与"发给手机"的文件夹（均 gitignore）
```

`AGENTS.md` 里有三条硬规矩：**bug 先说清现象再改、新功能先讨论（给方案与代价）、不加自动/常驻行为**。
它同时也是一份"开工顺序"清单（先说不足 → 代码纪律 → 变更记录 → docs），省得每次重新摸索。
...
>
> ⚠ **下面每一条都还没做，而且做之前会先在 issue 里讨论**：这是个人项目，取舍优先 ——
> 有些条目可能永远不做（比如为了兼容老机型把 `minSdk` 下探，代价是到处加兼容分支）。
> 想加功能也一样：**先说清"要解决谁的什么问题"**，比"这个能做"重要得多（见 `AGENTS.md`）。
